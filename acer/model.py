    ...
    def __init__(self, sess, policy, dynamics, ob_space, ac_space, nenvs, nsteps, ent_coef, q_coef, gamma, max_grad_norm, lr,
        ...
        with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
            ...
            if self.dynamics.dummy:
                ...
                if goal_as_image:
                    train_goal_placeholder = tf.placeholder(ob_space.dtype, (nenvs*nsteps,) + ob_space.shape, "train_goal")
                    concat_on_latent, train_goal_encoded = False, None
                else:
                    train_goal_placeholder = tf.placeholder(tf.float32, (nenvs*nsteps,) + goal_shape, "train_goal")
        ...
        qret = q_retrace(self.R, self.D, q_i, self.V_NEXT, rho_i, nenvs, nsteps, gamma)
